-- A tenant with more than one location (D-125).
--
...
comment on table tenant_branches is
  ...
  'otherwise the tenant-wide rows are the whole story, exactly as before 0047. D-125.';
...
comment on table branch_contact_points is
  'A branch''s contact point where it differs from contact_points. D-125.';
...
comment on table branch_hours is
  'A branch''s hours for one weekday where they differ from business_hours. D-125.';
...
comment on table branch_variant_prices is
  'A branch''s price for one service variant where it differs from service_variants. D-125.';
...
values ('clarify_branch',
        'Asks which branch the customer means, when the answer differs by branch and they have not said. D-125.')
