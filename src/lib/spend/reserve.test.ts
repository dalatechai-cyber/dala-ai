...
const input = { tenantId: TENANT, surface: 'reception' as const, estimate: 12_000_000n, now: NOW };
...
test('a reservation asks for every counter in ONE call', async () => {
  ...
  assert.deepEqual(reserveCalls[0]?.args['p_targets'], dayTargets(TENANT, NOW));
});
...
test('DONE-TEST: release GIVES THE BUDGET BACK, with the same targets and amount', async () => {
  ...
  await release(db, { id: 'res-1', tenantId: TENANT, surface: 'reception', estimate: 12_000_000n }, NOW);
  ...
  assert.deepEqual(call?.args['p_targets'], dayTargets(TENANT, NOW));
