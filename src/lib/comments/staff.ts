 ...
 *     person being answered («Saruul Naran Баярлалаа💕»), which is why (b) also covers the
 ...
 * further in («Bayrsaihan Jigjiddorj Ooyo Oyunchimeg 📍 Яармаг салбар…»). No `\b`, no `\w`:
 * a word here is a run between separators as Unicode defines them, so «Од» does not match
 * inside «Одгэрэл» and «Ooyo Oyunchimeg» matches against «Ooyo Oyunchimeg📍».
 *
 ...
 * thanking someone (webhook 484, «Enkhnasan Nasandalai Баярлалаа💕») is not the Page, and the
