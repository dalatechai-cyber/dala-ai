...
export type KbProvenance = {
  ...
  /**
   * Active branches left out because they are not `tenant_confirmed` (0047, D-125). A branch
...
export async function loadTenantKb(
  ...
  const [
    ...
    // 0047 (D-125). Read on every publish, so a publish against a project where 0047 is not
