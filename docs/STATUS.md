...
| **A QStash schedule for `/api/workers/digest`** | **ONE schedule, `5 16 * * *` UTC = 00:05 Ulaanbaatar** (D-128 addendum, founder 2026-09-26: it reaches him during his US day). If the older `0 1 * * *` (09:00) schedule still exists, delete it, or every report arrives twice. Either way the report covers the Ulaanbaatar day that has just ended. The cadence lives in the QStash console, not in this repository — `vercel.json` carries no `crons` block, exactly as with the hourly purge. Until it is scheduled the digest never runs, and an `on_change` condition that has gone quiet has nothing keeping it visible |
| **`DAILY_REPORT_V2=true` and `DAILY_REPORT_SECRET` in Vercel** (optional, D-128) | Unset = today's digest exactly. `true` sends one merged report (dalatech-app's section, the digest, «Yesterday», the flaw report) and moves non-actionable warnings into it. The secret must match dalatech-app's; preflight refuses `true` without it. `DAILY_REPORT_SECTION_URL` overrides the endpoint and is normally unset |
...
| ~~5f~~ | ~~`supabase db push` for `0021`~~ | **Pushed and verified 2026-09-07.** The ledger reads twenty-one rows, newest `0021`. Verified beyond the count: `ops.purge_expired`'s `prosrc` is byte-identical to the one the repo's migrations build on a scratch cluster (md5 `2fe7be61…`, 3,633 chars), `public.purge_expired` exists as a wrapper, SECURITY DEFINER is on the `ops` function only, and both ACLs are `postgres` and `service_role` |
| ~~5g~~ | ~~Matrix's nine `canned_responses`~~ | **Approved as drafted and inserted 2026-09-07** (`scripts/provision/matrix-stage3-canned.sql`). All nine share one `reviewed_at`, `reviewed_by = 'founder'`; every body read back md5-identical to the reviewed text |
| ~~5h~~ | ~~Matrix's `refusal_out_of_scope` line~~ | **Approved and inserted 2026-09-07.** Eleven canned lines now, all sharing one `reviewed_at` |
| ~~5i~~ | ~~Matrix's children's-services rule~~ | **Approved and inserted 2026-09-07** as a `disclosure_rules` row (`scripts/provision/matrix-stage3b-children.sql`). Ш1 was already the gate |
| ~~5k~~ | ~~A decision on opening hours~~ | **Decided by the founder and built 2026-09-07:** `business_hours` renders into an L3 section, so the rows stay the single source and the digits reach `allowed_numbers`. Building it found that a model writing «10:00-20:00» without spaces produced ONE fused numeral no allow-list could hold — fixed in `extractNumerals` (D-051 has the detail). Heading is «БАЙГУУЛЛАГЫН АЖЛЫН ЦАГ», deliberately not «АЖЛЫН ЦАГ», which is one word from Ш4's own subject |
| ~~5l~~ | ~~Compile and publish Matrix's configuration~~ | **Published 2026-09-07** — seq 1, `content_hash 4d0b4243…`, 12,313 chars, 18 sections, fourteen allowed numbers, pointer moved, stored prefix re-hashes to the compiled hash. Publishing found two bugs; see D-051 |
| 5m | **`tenant_budgets` row for Matrix** | **The first hard blocker for `shadow`, and it is money, so it is yours.** `shadow` is `{deliver: false, generate: true}`, so it calls the model and `reserve_spend` runs first. Matrix has no budget row, so every reply would refuse with `guard_unavailable` and 503. D-004's formula gives ₮80,000 ≈ $22.86/mo against measured spend of $16.57 (D-016) |
| 5n | **`tenant_roles` row for Matrix** (`reception`, entitled) | The second blocker, and it is not money: `withTenantRole` returns 403 `role_not_entitled` when the tenant has no entitled row for the role. Matrix has none. Not gated by any of the four categories — say the word and it is one INSERT |
| 5o | **A sealed `page_token` for Matrix** | Needed for `live`, NOT for `shadow` — a shadow reply is generated and withheld, so nothing reaches Graph. `token_status` is `unprovisioned` and `live_requires_active_token` will refuse `live` until it is sealed. Credentials, so yours |
| ~~0022~~ | **`supabase db push` for `0022`** | Fixes `publishRevision`'s supersede step, which refused for every revision that had ever been live — so every tenant's SECOND publish failed halfway. Matrix's first publish did not need it; the second will. See D-051 |
| ~~5j~~ | ~~Matrix's booking URL~~ | **Confirmed by the founder and written 2026-09-07** — `tenant_booking` `link` / `https://www.matrixecosalon.org/` (`matrix-stage4b-booking.sql`). It also feeds `allowedUrls` and `scriptShareExclusions`, so a model paraphrase carrying the URL is no longer refused as `outbound_url` |
| ~~8c~~ | ~~One QStash schedule → `/api/workers/purge`~~ | **Created and FIRING 2026-09-07, HOURLY** (the founder changed it from nightly after the first report). The cadence is not in this repository — `vercel.json` has no `crons` block and the schedule lives in the QStash console — so ask rather than infer it. Hourly makes the 1-day floor on unrouted events ~1 hour of slack rather than ~24. Still doing nothing visible, correctly: at 15:4x UTC on the 7th the oldest row is 38.4h old against a 7-day tenant retention, and events 9 and 10 are 14.5h old against their 1-day floor, so the first due row is 9 at ~01:12 on the 8th |
...
| 14 | Seal the Page token | **Done, and the round trip is closed.** One `tenant_secrets` row, `page_token`, `status=active`, sealed under KEK **v1**, `last_ok_at = 2026-09-19 12:16:18.117+00` — the runtime loader has opened it over real PostgREST, which is the transport CI cannot exercise. This cell read "`last_ok_at` is null — it has never been opened" until 2026-09-21, when the founder corrected it. **v1 can now never be removed from the environment and its value cannot be read back out of Vercel (D-107)** |
...
| 15 | Subscribe the app to the Page, and **verify the app-level subscription too** | **BROKEN SINCE 2026-09-07 01:12 UTC, cause not yet read (D-062).** A page-level subscribe returns `{"success": true}` even when the app has never enabled that field, and no events are ever delivered. The 2026-09-06 deliveries proved the subscription worked for `messages` on this Page **then**; nothing has arrived for either Page since 01:12 on 2026-09-07, and the endpoint, the custom domain and Meta itself are all measured healthy. Run `node scripts/diagnose/meta-subscription.ts --app-id 1562862634970492 --app-id 1380702870025418 --field messages --field feed` with `META_APP_SECRETS` in the shell (`--field` is required and has no default: it used to be a `messages` constant, which would have reported healthy with `feed` off) — it prints the callback URL, `active` and the field list Meta currently holds |
| 15b | **Matrix Stage 4: the knowledge base** | **DONE 2026-09-07** (D-047). Nine staff (six active), eight services, seven knowledge documents, one contact, one refusal topic. Rendering these rows gives five sections and `hasTenantData` true, so D-033's guard will not fire **once the config is compiled** — `config_revisions` is still zero rows for this tenant, so nothing reads them at runtime yet. **Blank and reported:** all prices, `faqs`, `disambiguation_pairs` («цаг»), deposits, booking URL, staff tiers. **Stage 3 still blocks generation** — Matrix has zero `canned_responses`, and without a `handoff` row every refusal retries for ever |
| 16 | Subscribe `DALA_AI` to **Matrix's** Page alongside `dalatech`, then `delivery_mode` | **THE MIRROR PRODUCED ZERO DATA AND THE FOURTEEN DAYS ARE GONE (D-062).** Two deliveries naming Matrix's Page reached this platform at 01:12 UTC on 2026-09-07 — `unrouted`, correctly, the channel row did not exist until 01:23 — and nothing has arrived since, for this Page or tenant #0's. Both stopped inside forty-one minutes, and all seven deliveries on record verified against the same entry in `META_APP_SECRETS`, so it is one app-level fault rather than two. Item 15 is the read that says which. **The mirror restarts from zero once it is fixed; no part of the fourteen days was measured.** The rest of this cell still holds: **the mirror is a second subscription — measured (D-043), no forwarding hop.** §3.13.1 is the exact call and the four-read pre-flight; the abort check is `debug_token`'s `app_id`, because `subscribed_apps` takes no app parameter and a `dalatech`-issued token would rewrite the incumbent's field list on a live Page. Never through the console's Add Page picker: it writes the complete Page set and took Matrix offline for ten minutes on 2026-09-06. **Tenant #0 is `live`** (`status active / delivery_mode live / token_status active`, `name_confirmed_at` set). `token_status = 'active'` was an ASSERTION when it was set and is no longer: `last_ok_at` reads 2026-09-19 12:16:18.117+00, so the runtime has opened that token. **Matrix is different** — its 14-day mirror runs on `shadow`, which drafts and does not send. **Not `live`** |
