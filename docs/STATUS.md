...
| 5h | **Matrix's `refusal_out_of_scope` line** | **The new blocker, and it is not optional.** `matrix-stage4-kb.sql` wrote a `photo_consultation` rule pointing at a kind Matrix has no row for. Since D-050 that refuses with `canned_response_missing` instead of letting the model improvise, so Matrix cannot publish until the line exists. Drafted in `scripts/provision/drafts/matrix-children-services.sql`, not inserted |
| 5i | **Matrix's children's-services rule** | A regression against their incumbent, not a new feature: their production bot refuses children's prices outright and ours would answer. Ш1 is already the gate; what is missing is one `disclosure_rules` row and the `refusal_topic` line. Drafted alongside 5h, not inserted |
| 5k | **A decision on opening hours — yours, because it changes what a customer sees** | Found 2026-09-07 by probing the guard, not by reading code. `business_hours` holds all seven of Matrix's days correctly, and **nothing puts them in front of the model**: L4 renders only «ОДОО: НЭЭЛТТЭЙ / ХААЛТТАЙ», never the schedule, and none of the seven knowledge documents contains a clock time. So «Хэдэн цагт ажилладаг вэ?» — one of the commonest questions a salon gets, and one their incumbent answers from its FAQ — takes the handoff line. The guard makes it doubly certain: `10:00-20:00` reduces to digits that are in no tenant section, so it would be refused as `outbound_price` even if the model guessed right. Three shapes, and the choice is a product one: a `knowledge_documents` row (duplicates the schedule in prose, two places to disagree); a `deterministic_replies` row (free, no model call, but a pinned Mongolian sentence); or render `business_hours` into an L3 section so the rows stay the single source and the numerals reach `allowed_numbers` (the config-not-code answer, and the only one that also fixes tenant #3). Not built — it changes what a customer sees |
