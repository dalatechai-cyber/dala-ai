...
| 15b | **Matrix Stage 4: the knowledge base** | **DONE 2026-09-07** (D-047). Nine staff (six active), eight services, seven knowledge documents, one contact, one refusal topic. `hasTenantData` is true, so D-033's guard no longer fires for them. **Blank and reported:** all prices, `faqs`, `disambiguation_pairs` («цаг»), deposits, booking URL, staff tiers. **Stage 3 still blocks generation** — Matrix has zero `canned_responses`, and without a `handoff` row every refusal retries for ever |
