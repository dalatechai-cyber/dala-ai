...
| | |
|---|---|
| `supabase/migrations/0001`–`0017` | apply clean in order on an empty database (`scripts/localvalidate/run.sh`) |
...
| `scripts/verify/catalog.sql` | **29/29 PASS** on PG16. On the real project V25 fails by design until `supabase_admin`'s default ACL is revoked by a role that can; V26 and V27 pass since `0014` and `0015` landed; V28 fails until `0017` is pushed |
