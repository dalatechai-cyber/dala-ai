 ...
 *
 * `0010` is applied to the project, so this file is history: regenerating it with new rows
 * would produce a migration that no longer matches what ran. Per-vertical blocks therefore
 * go to their own migration (`buildVerticalSeedSql`), and **editing the text of a block
 * that is already in `0010` needs a new migration too** — the generator cannot express
 * that, and re-running it would silently rewrite an applied file.
 */
export function buildSeedSql(root = process.cwd()): string {
  const blocks = readSignedBlocks(root).filter((b) => b.vertical === null);
  const values = blocks
    .map((b) => {
...
export const VERTICAL_SEED_PATH = 'supabase/migrations/0019_prompt_blocks_vertical_seed.sql';

const VERTICAL_HEADER = `-- 0019 — the signed per-vertical platform blocks (0018, D-034).
--
-- GENERATED by scripts/prompt/generate-seed.ts, from the \`prompt/platform/*.<vertical>.mn.txt\`
-- files and their sign-offs. Do not edit by hand; \`generate-seed.test.ts\` regenerates it
-- and fails on any difference.
--
-- Separate from 0010 because 0010 is applied to the project and a generated file that
-- rewrites an applied migration is a file that no longer describes what ran. The conflict
-- target is the index 0018 relaxed: one row per (block_key, vertical).

`;

/** The per-vertical blocks, or null when none are signed yet. */
export function buildVerticalSeedSql(root = process.cwd()): string | null {
  const blocks = readSignedBlocks(root).filter((b) => b.vertical !== null);
  ...
  const values = blocks
    .map((b) => {
      ...
      return (
        `  ('platform', null, '${b.blockKey}', ${b.ordinal}, ${layer},\n` +
        `   ${TAG}${b.body}${TAG},\n` +
        `   '${b.reviewedBy}', '${b.reviewedAt}'::timestamptz, '${b.vertical ?? ''}')`
      );
    })
    .join(',\n');
...
if (process.argv[1] !== undefined && process.argv[1].endsWith('generate-seed.ts')) {
  const shared = buildSeedSql();
  const perVertical = buildVerticalSeedSql();
  const check = process.argv.includes('--check');

  const compare = (file: string, want: string | null): boolean => {
    const exists = existsSync(file);
    if (want === null) {
      if (!exists) return true;
      process.stderr.write(`${file} exists but no signed per-vertical block does. Delete it.\n`);
      return false;
    }
    if (exists && readFileSync(file, 'utf8') === want) return true;
    process.stderr.write(`${file} is stale. Run: node scripts/prompt/generate-seed.ts\n`);
    return false;
  };

  if (check) {
    const ok = compare(SEED_PATH, shared) && compare(VERTICAL_SEED_PATH, perVertical);
    if (!ok) process.exit(1);
    process.stdout.write('the generated seed migrations are up to date.\n');
  } else {
    writeFileSync(SEED_PATH, shared);
    process.stdout.write(`wrote ${SEED_PATH}\n`);
    if (perVertical === null) {
      process.stdout.write(`no signed per-vertical block; ${VERTICAL_SEED_PATH} not written\n`);
    } else {
      writeFileSync(VERTICAL_SEED_PATH, perVertical);
      process.stdout.write(`wrote ${VERTICAL_SEED_PATH}\n`);
    }
  }
}
